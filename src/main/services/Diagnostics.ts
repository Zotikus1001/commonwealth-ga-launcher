...
export function buildDiagnosticsReport(state: LauncherState, settings: Settings, logTail: string[]): string {
  ...
  const lines: string[] = [
    ...
    `DXVK/Vulkan: ${state.dxvk.status} (version=${state.dxvk.version}, canRestore=${state.dxvk.canRestore})`,
    ...
    `game profiles: ${state.gameProfiles.length} (enabled=${state.gameProfilesEnabled}, selected=${selectedProfile?.name ?? 'none'})`,
